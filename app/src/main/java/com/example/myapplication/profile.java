...
public class profile extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        goal = v.findViewById(R.id.profileGoal);

        SharedPreferences mPref = getContext().getSharedPreferences("user_profile", getContext().MODE_PRIVATE);
        String profileName = mPref.getString("name", "John Doe");
        int profileAge = mPref.getInt("age", 20);
        int profileGoal = mPref.getInt("goal", 2000);

        name.setText(profileName);
        age.setText(profileAge+ "");
        goal.setText(profileGoal+ "");
        return v;
    }
}